...
本構成では、オンプレミス相当の Active Directory（AD DS）を起点に、
Microsoft Entra ID へ Cloud Sync を用いたハイブリッド ID 連携を行う。

- AD DS は Azure Bastion 経由でのみ管理し、Public IP を持たない
- Entra ID を中核として、SSO / SCIM / Conditional Access / PIM を実装
- SaaS（Grafana / WordPress / ServiceNow）と安全に統合
- サインインログ、監査ログ、プロビジョニングログ、Bastion 操作ログは
  Log Analytics Workspace に集約し、可観測性を確保する
